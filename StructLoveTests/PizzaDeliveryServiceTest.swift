...
class PizzaDeliveryServiceSpec: QuickSpec {
    override func spec() {
        var service = PizzaDeliveryService(location: Coordinate(latitude: 50.0, longtitude: 10.0))

        describe("initializer") {
            it("sets the struct's properties") {
                expect(service.location.latitude).to(beCloseTo(50.0))
                expect(service.location.longtitude).to(beCloseTo(10.0))
        ...
        describe("isInRange()") {
            it("returns true if the destination is in range of the service") {
                let dest = Coordinate(latitude: 45.0, longtitude: 10.0)
                expect(service.isInRange(dest)).to(beTrue())
            }

            it("returns false if the destination is not in range of the service") {
                let dest = Coordinate(latitude: -30.0, longtitude: 10.0)
                expect(service.isInRange(dest)).to(beFalse())
            }
        }
